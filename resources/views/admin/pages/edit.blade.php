...
@section('title', __('admin/pages.edit_page'))
...
@section('content_header')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('admin/pages.edit_page') }}: {{ $page->title }}</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">{{ __('admin/pages.edit_page') }}</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">{{ __('admin/pages.edit_page') }}: {{ $page->title }}</h3>
                    </div>
                    <form action="{{ route('admin.pages.update', $page) }}" method="POST">
                        @csrf
                        @method('PUT')
                        @include('admin.pages.form', ['languages' => $languages, 'descriptions' => $descriptions])
                    </form>
                </div>
            </div>
        </div>
