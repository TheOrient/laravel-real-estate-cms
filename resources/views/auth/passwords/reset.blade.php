    ...
    <div class="flex-1 flex items-center justify-center px-6 py-32">
        <div class="w-full max-w-md">
            <div class="text-center mb-8">
                <h2 class="text-3xl font-bold text-[#1A1A1A] mb-2" style="font-family: Inter, sans-serif;">{{ __('auth.reset_password_title') }}</h2>
                <p class="text-sm text-[#666666]" style="font-family: Inter, sans-serif;">{{ __('auth.reset_password_subtitle') }}</p>
            </div>
                ...
                <form class="space-y-6" action="{{ route('password.update') }}" method="POST">
                    ...
                    <div>
                        ...
                        <input id="email" name="email" type="email" required
                            ...
                            placeholder="{{ __('auth.email_placeholder') }}" value="{{ $email ?? old('email') }}" style="font-family: Inter, sans-serif;">
                        @error('email')
                    ...
                    <div>
                        <label for="password" class="block text-sm font-semibold text-[#1A1A1A] mb-2" style="font-family: Inter, sans-serif;">{{ __('auth.new_password') }}</label>
                        <input id="password" name="password" type="password" required
                            class="w-full px-4 py-3 border border-[#E0E0E0] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E6F5C] focus:border-transparent transition-all @error('password') border-red-500 @enderror"
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-sm font-semibold text-[#1A1A1A] mb-2" style="font-family: Inter, sans-serif;">{{ __('auth.new_password_confirmation') }}</label>
                    ...
                    <button type="submit"
                            ...
                            style="font-family: Inter, sans-serif; font-weight: 700;">{{ __('auth.reset_password_button') }}</button>
                </form>
